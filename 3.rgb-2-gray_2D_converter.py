...
def gpu_rgb_2_gray_2D(img):
    ...
    @cuda.jit
    def kernel_convert(src, dst, x_size, y_size):
        tidx = cuda.threadIdx.x + cuda.blockIdx.x * cuda.blockDim.x
        tidy = cuda.threadIdx.y + cuda.blockIdx.y * cuda.blockDim.y
        if (tidx >= x_size or tidy >= y_size):
            return
        pos = tidy * x_size + tidx
        sum = src[pos * 3] + src[pos * 3 + 1] + src[pos * 3 + 2]
        dst[pos] = np.uint8(sum / 3)
    ...
    x_pixel = img.shape(0)
    y_pixel = img.shape(1)
    thread_per_block = (64, 64)

    def compute_block(pixel, id):
        return (pixel + thread_per_block[id] - 1) // thread_per_block[id]
    block_per_grid = (compute_block(x_pixel, 0), compute_block(y_pixel, 1))

    debug(2, "start execute thread in gpu...")
    kernel_convert[block_per_grid, thread_per_block](
        dev_input, dev_output, x_pixel, y_pixel)
...
def gpu_rgb_2_gray(img):
    ...
    def kernel_convert(src, dst, size):
        ...
        sum = src[tidx * 3] + src[tidx * 3 + 1] + src[ + 2]
...
if __name__ == "__main__":
    ...
    print("GPU 2D Running...")
    start = time.time()
    img = gpu_rgb_2_gray(image)
    end = time.time()
    print(f"Elapsed time: {(end - start) * 1000} ms")
    img_name = f"{image_file.split('/')[-1].split('.')[0]}_gpu_2D_gray.jpg"
    save_img(img_name, img)
